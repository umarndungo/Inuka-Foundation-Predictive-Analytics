...
async def bootstrap_alerts_from_graph(
    ...
    for row in rows:
        alert = Alert(
            ...
            severity="critical" if row.risk_tier == "HIGH" else "medium",
            type="critical_risk" if row.risk_tier == "HIGH" else "high_risk",
            status="new",
            description=f"Beneficiary {row.beneficiary_id} flagged {row.risk_tier} risk from identity graph bootstrap.",
            location=row.region or "Unknown",
            device_id=None,
            alert_metadata={"source": "bootstrap_from_graph"},
